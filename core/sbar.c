...
__IRAM int			sb_updates;		// if >= vid.numpages, no update needed

#define STAT_MINUS		10	// num frame for '-' stats digit
__IRAM qpic_t		*sb_nums[2][11];
__IRAM qpic_t		*sb_colon, *sb_slash;
__IRAM qpic_t		*sb_ibar;
__IRAM qpic_t		*sb_sbar;
__IRAM qpic_t		*sb_scorebar;

__IRAM qpic_t      *sb_weapons[7][8];   // 0 is active, 1 is owned, 2-5 are flashes
__IRAM qpic_t      *sb_ammo[4];
__IRAM qpic_t		*sb_sigil[4];
__IRAM qpic_t		*sb_armor[3];
__IRAM qpic_t		*sb_items[32];

__IRAM qpic_t	*sb_faces[7][2];		// 0 is gibbed, 1 is dead, 2-6 are alive
							// 0 is static, 1 is temporary animation
__IRAM qpic_t	*sb_face_invis;
__IRAM qpic_t	*sb_face_quad;
__IRAM qpic_t	*sb_face_invuln;
__IRAM qpic_t	*sb_face_invis_invuln;

__IRAM qboolean	sb_showscores;

__IRAM int			sb_lines;			// scan lines to draw
...
__IRAM int		fragsort[MAX_SCOREBOARD];

char	scoreboardtext[MAX_SCOREBOARD][20];
__IRAM int		scoreboardtop[MAX_SCOREBOARD];
__IRAM int		scoreboardbottom[MAX_SCOREBOARD];
__IRAM int		scoreboardcount[MAX_SCOREBOARD];
__IRAM int		scoreboardlines;
