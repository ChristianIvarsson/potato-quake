...
__IRAM int    c_faceclip; // number of faces clipped



__IRAM static unsigned int	cacheoffset;
...
__IRAM static medge_t    *r_pedge;

__IRAM static qboolean    r_leftclipped, r_rightclipped;
__IRAM static qboolean    makeleftedge, makerightedge;
__IRAM static qboolean    r_nearzionly;
...
__IRAM static int				r_emitted;
static float			r_nearzi;
static float			r_u1, r_v1, r_lzi1;
__IRAM static int				r_ceilv1;

__IRAM static qboolean	r_lastvertvalid;
...
void R_EmitCachedEdge (void)
{
	...
	if (((float)tmp) > r_nearzi)	// for mipmap finding
		r_nearzi = ((float)tmp);
...
// if no edges made it out, return without posting the surface
	...
	surface_p->flags = fa->flags | insubmodel;
...
// if no edges made it out, return without posting the surface
	...
	surface_p->nearzi = r_nearzi;
	surface_p->flags = psurf->flags | SURF_INSUBMODEL;
	surface_p->spanstate = 0;
	surface_p->entity = currententity;
